...
def main():
    ...
    param_grid = [{'alpha':np.logspace(-10, 2, 50),  'gamma':np.linspace(1, 100, 50), 'kernel': ['poly'], 'degree':np.linspace(1, 4, 4)}]
    model = GridSearchCV(kernel_ridge.KernelRidge(), param_grid, cv=10, scoring=None, fit_params=None, n_jobs=-1, iid=True, refit=True, verbose=1, pre_dispatch='2*n_jobs', error_score='raise', return_train_score=True)
